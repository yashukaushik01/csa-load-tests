...
export default function postShippingLabelRequest() {
  ...
  group('Post Shipping Label request', () => {
    let unique_request_id_forward_label = `lturifl${randomString(29, 'abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ')}`;
    ...
    let body = `{
      ...
      "UniqueRequestId": "${unique_request_id_forward_label}",
      ...
      "Options": {
        "LOCATION CODE": "MIA-MIA-OE|USPS-PR-V2"
      }
    }`;
    ...
    if (post_shipping_label_request.status != 200) {
      file.appendString(failed_data_file_path, `'${unique_request_id_forward_label}','${post_shipping_label_request.status}','${post_shipping_label_request.body}'\n`);
